...
package body Bitcoin.Crypto is
  ...
  -- Ignore/Assert --
  -------------------
  procedure Ignore (Result : in BIGNUM_Access) is begin null; end;
  ...
  -- To_Byte_Array --
  -------------------
  function To_Byte_Array (Item : in BIGNUM_Access) return Byte_Array is
  ...
  function To_Address is new Ada.Unchecked_Conversion (BIGNUM_Access, Address);
  function To_Address is new Ada.Unchecked_Conversion (EC_KEY_Access, Address);

  -----------------------
  -- Derive_Public_Key --
  -----------------------
  procedure Derive_Public_Key (Key_Pair : in out Key_Pair_Type; Private_Key : in out Byte_Array) is
    Priv_Key : BIGNUM_Access   := BN_bin2bn (Private_Key (Private_Key'First)'Unchecked_Access, Private_Key'Length, null);
    Group    : EC_GROUP_Access := EC_KEY_get0_group (Key_Pair.Ptr);
    Pub_Key  : EC_POINT_Access := EC_POINT_new (Group);
  begin
    Put_Line ("1");
    Put_Line (Address_Image (To_Address (Priv_Key)));
    Put_Line (Address_Image (To_Address (Key_Pair.Ptr)));
    Put_Line (Address_Image (To_Address (EC_KEY_get0_private_key (Key_Pair.Ptr))));
    Assert (EC_KEY_set_private_key (Key_Pair.Ptr, Priv_Key));
    Put_Line ("2");
    declare Context : Big_Number_Context; begin
      Put_Line ("3");
      Assert (EC_POINT_mul (Group, Pub_Key, Priv_Key, null, null, Context.Ptr));
      Put_Line ("4");
  ...
  function Get_Private_Key (Key_Pair : in Key_Pair_Type) return Byte_Array is
    Private_Key : BIGNUM_Access := EC_KEY_get0_private_key (Key_Pair.Ptr);
  ...
  function Get_Public_Key (Key_Pair : in Key_Pair_Type; Format : in Point_Format_Kind) return Byte_Array is
    Point      : EC_POINT_Access := EC_KEY_get0_public_key (Key_Pair.Ptr);
    Group      : EC_GROUP_Access := EC_KEY_get0_group (Key_Pair.Ptr);
    Public_Key : BIGNUM_Access;
